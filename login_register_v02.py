...
def login(username, password, window):
    ...
    #     createWindow()
    # elif result[0][0] == 1:
    #     query = "SELECT username FROM employees WHERE username = %s AND password = %s"
    #     connect.cursor.execute(query, (username, password))
    #     result = connect.cursor.fetchall()
    #     name = result[0][0]
    #     window.destroy()
    #     employeeInterface(globals()[username]) 
    if password == globals()[username].password and username == globals()[username].username:
        employeeInterface(username)
    else:
        messagebox.showerror('Invalid Credentials', 'Wrong Username or Password, please try again')
...
def employeeInterface(fname):
    ...
    welcome = Label(if1, text=f'Welcome, {globals()[fname].name}', font=('Arial', 15), border=3)
    ...
    assignedTasks = globals()[fname].getAssignedTasks() 
    if all(assignedTask.completed == 1 for assignedTask in assignedTasks) or len(assignedTasks) == 0:
        notasks = Label(main_taskframe, text='You have not active tasks')
        notasks.pack()
    for assignedTask in assignedTasks:
        if assignedTask.completed != 1:
            task = Button(main_taskframe, text=assignedTask.title, height=5, width=15, cursor='hand2',  command= lambda assignedTask=assignedTask: globals()[fname].view_assigned_task(assignedTask))  # Adjust height and width as needed
            task.pack()
    ...
    meetings = globals()[fname].getMeetings()  
    if len(meetings) == 0:
        nomeets = Label(meetframe, text='You have not active meetings')
        nomeets.pack()
    for meeting in meetings:
        task = Button(meetframe, text=meeting.meetingName, height=5, width=15, cursor='hand2', command= lambda meeting=meeting:globals()[fname].view_meeting_schedule(meeting))  # Adjust height and width as needed
        task.pack()
    ...
    # Completed Tasks
    comTasks = Button(if1, text='Show Completed Tasks', fg='white', bg='green', cursor='hand2', command= globals()[fname].show_completed_tasks)
...
def register(fname, lname, username, password, password2, window):
    if password != password2:
        ...
        # SQL QUERY
        globals()[username] = Employee(fname+" "+lname, any, username, password, '', '', [], [], [])
        # query = "INSERT INTO employees VALUES (null, %s, %s, %s)"
